...
async function fetchWeather(city) {
  const url = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&appid=${apiKey}&units=metric&lang=kk`;
  let res;
  try {
    res = await fetch(url);
  } catch {
    throw new Error('Желіде ақау бар');
  }
  if (!res.ok) {
    if (res.status === 404) throw new Error('Қала табылмады');
    throw new Error(`Қате код: ${res.status}`);
  }
  return res.json();
}
